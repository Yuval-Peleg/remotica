...
- `src/pages/` — route-level components (`Dashboard.jsx` and `Settings.jsx` are real; `System`/`About` are still placeholders). `Settings.jsx` is where the printer gets identified, rebuilt (2026-08-03) around two panels because the previous single-form version read as one undifferentiated pile of numbers:
  - **"Suggested printer"** — connection badge, then what `GET /api/printer-suggestion` reports the attached printer is, with the raw `M115` reply shown underneath as the *evidence* for that guess (labelled "What the printer reported"). Showing the evidence is the point: the closing note tells the user Remotica only inferred this from a model name and they should confirm it themselves, which they can't do if the basis is hidden. Purely informational — this panel never applies anything.
